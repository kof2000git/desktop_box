using System.Collections.Generic;
using System.Windows;

namespace DesktopBox.Controls;

/// <summary>获取所有显示器的工作区边界(含多屏拼接),用于盒子边缘磁吸。</summary>
public static class SystemParametersHelper
{
    public static IReadOnlyList<Rect> AllScreens => GetScreens();

    public static double LayoutWidth
    {
        get
        {
            var screens = AllScreens;
            if (screens.Count == 0) return SystemParameters.PrimaryScreenWidth;
            return screens.Max(s => s.Right) - screens.Min(s => s.Left);
        }
    }

    private static List<Rect> GetScreens()
    {
        var list = new List<Rect>();
        try
        {
            foreach (var s in System.Windows.Forms.Screen.AllScreens)
                list.Add(new Rect(
                    s.WorkingArea.X, s.WorkingArea.Y,
                    s.WorkingArea.Width, s.WorkingArea.Height));
        }
        catch
        {
            list.Add(SystemParameters.WorkArea);
        }
        return list;
    }

    /// <summary>约束盒子左上角:保证 keepW×keepH 的区域留在某个屏幕工作区内。
    /// 防止拖到屏外、或加载到已失效的旧坐标(换分辨率/拔屏幕)后窗口找不到。返回应使用的 (X,Y)。</summary>
    public static (double X, double Y) ClampIntoScreens(double x, double y, double keepW = 120, double keepH = 40)
    {
        var screens = AllScreens;
        if (screens.Count > 0)
            return ClampIntoScreens(x, y, keepW, keepH, screens);

        // 兜底:主工作区
        var wa = SystemParameters.WorkArea;
        return ClampIntoScreen(x, y, keepW, keepH, wa);
    }

    public static (double X, double Y) ClampIntoScreens(double x, double y, double keepW, double keepH, IReadOnlyList<Rect> screens)
    {
        if (screens.Count == 0)
            return (x, y);

        var best = screens
            .Where(s => s.Width >= keepW && s.Height >= keepH)
            .DefaultIfEmpty(screens[0])
            .MinBy(s => DistanceSquaredToRect(x, y, s));

        return ClampIntoScreen(x, y, keepW, keepH, best);
    }

    private static (double X, double Y) ClampIntoScreen(double x, double y, double keepW, double keepH, Rect screen)
    {
        var maxX = Math.Max(screen.Left, screen.Right - keepW);
        var maxY = Math.Max(screen.Top, screen.Bottom - keepH);
        return (Math.Clamp(x, screen.Left, maxX), Math.Clamp(y, screen.Top, maxY));
    }

    private static double DistanceSquaredToRect(double x, double y, Rect rect)
    {
        var dx = x < rect.Left ? rect.Left - x : x > rect.Right ? x - rect.Right : 0;
        var dy = y < rect.Top ? rect.Top - y : y > rect.Bottom ? y - rect.Bottom : 0;
        return dx * dx + dy * dy;
    }
}
